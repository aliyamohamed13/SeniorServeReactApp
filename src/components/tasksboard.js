...
class TasksBoard extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "http://localhost:8080/api/v1/task/incompleteTasks/username=" +
        this.props.username
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(console.log);
    fetch(
      "http://localhost:8080/api/v1/taskcompletion/allTask/username=" +
        this.props.username
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  handleClose = () => {
    this.setState({ showUpdateTask: false, showMarkComplete: false });
  ...
  handleMarkComplete = (task) => {
    ...
    this.setState({ Task_ID: task.Task_ID, Username: task.Username });
  };

  handleMarkCompletePost = () => {
    if (this.state.completeDate === "" || this.state.monetaryAmount === "") {
      alert("Please complete all fields of form");
    } else {
      var randomTaskID = Math.floor(Math.random() * 1000000000);
      var postUrl =
        "http://localhost:8080/api/v1/taskcompletion/taskCompletionRecord";
      axios
        .post(postUrl, {
            ...
            <div key={task.Task_ID} className="card">
              <div className="card-body">
                ...
                <p className="card-text">
                  {task.Address} {task.City} {task.Province} {task.PostalCode}
                </p>
                <Button
                  className="del-btn"
                  type="button"
                  onClick={() => this.handleDeleteTask(task.Task_ID)}
                  style={{ margin: "5px" }}
                >
                  Delete Task
                </Button>
                ...
                <Button
                  type="button"
                  onClick={() => this.handleMarkComplete(task)}
                  style={{ margin: "5px" }}
                >
                  Mark Task as Complete
                </Button>
                <Modal
                  show={this.state.showUpdateTask}
    ...
    if (this.state.userCompletedTaskInfo.length === 0) {
      ...
      completedTasks = (
        <MakeReview
          ...
          tasks={this.state.userCompletedTaskInfo}
        />
      );
    }
