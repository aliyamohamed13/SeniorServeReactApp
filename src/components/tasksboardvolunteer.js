...
class TasksBoardVolunteer extends Component {
	...
	componentDidMount() {
		fetch("https://seniorserve-spring-postgres.herokuapp.com/api/v1/taskrequest/allTask/username=" + this.props.username)
			.then(res => res.json())
			.then(data => {
				this.setState({ requestedTasks: data });
			})
		fetch("https://seniorserve-spring-postgres.herokuapp.com/api/v1/volunteer/acceptedTasks/username=" + this.props.username)
			.then(res => res.json())
			.then(data => {
