...
class TaskRequest extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "http://localhost:8080/api/v1/taskrequest/allRequestUser/task_id=" +
        this.props.taskID
    )
      .then((res) => res.json())
      .then((data) => {
          ...
          {this.state.pendingTaskRequests.map((request) => (
            <div key={request.username} style={{ marginBottom: 10 }}>
              ...
              <p>
                ...
                <br /> Hours as Volunteer: {request.totalHours}
              </p>
              <Button
                onClick={(event) => this.handleAcceptRequest(request.username)}
              >
                Accept Request
              </Button>
            </div>
